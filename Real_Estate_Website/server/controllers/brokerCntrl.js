...
export const deleteBroker = asyncHandler(async (req, res) => {
    ...
    const brokerExists = await prisma.properties.findUnique({ where: { id: id } });
...
export const updateBroker = asyncHandler(async (req, res) => {
    ...
    try {
        const updateBroker = await prisma.properties.findUnique({ where: { id: id } });
        ...
        if (!updateBroker) {
            ...
            res.status(200).json({ message: "Broker info updated successfully", broker: updatedBroker });
        }
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "An error occurred while updating Broker info" });
    }
});
